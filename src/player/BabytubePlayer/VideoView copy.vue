...
import { BabyTubeItemInfo, BabyTubeModule, PlayMode } from '../../store/Babytube';
...
export default class VideoView extends Vue {
    ...
    get video(): HTMLVideoElement{ return this.$refs.video }
    
    @Watch('videoInfo')
    onChangeVideoURL( curr, old ){
        gsap.to(document.getElementsByClassName("videoBox"),{opacity:0,duration:0})
        ...
        // if( isIOS() ){
            ...
            gsap.to(document.getElementsByClassName("videoBox"),{delay:1,opacity:1,duration:0.5})
    ...
    private _onVideoPlayEnd(){
        if(this.mLoopMode!="none"){ this.play(); }
    ...
    close(){
        gsap.to(this.$el,{height:0,duration:0.3})
