...
export class Clean extends PetSceneBase {
...
//   private mTextMove: gsap.core.Tween;
    ...
    async onInit() {
        this.mBG = new BGImage( "bg_00.png" );
    ...
    _setupPlayerAni(){
        this.mPlayer.state.setAnimation( 0, "03_cleaning/03_enter", false );
        this.mPlayer.state.addListener({
           
            complete: (entry) => {
                
                // console.log( "Ani Finish:", entry.animation.name);
                if (entry.animation.name == "03_cleaning/03_enter") {
                    this.mPlayer.state.setAnimation(0, "03_cleaning/03_ready", true);
                    const temp = this.mPlayer.slotContainers[ 
                        this.mPlayer.skeleton.findSlotIndex("03_click") 
                    ];
                    ...
                    temp.on("pointerdown",()=>{
                        ...
                        console.log("click");
                        this.mPlayer.state.setAnimation(0, "03_cleaning/03_action", false);                    
                    })
                }else if (entry.animation.name == "03_cleaning/03_action") {
                    this.game.dogCondition.needClean -= 1;
                    if( this.game.dogCondition.needClean < 0 ) this.game.dogCondition.needClean = 0;
                    ...
                    if( this.game.dogCondition.needClean>0){
                        ...
                        this.mPlayer.state.setAnimation(0, "03_cleaning/03_reaction", false); 
                    }

                }else if (entry.animation.name == "03_cleaning/03_reaction") {
                    this.game.goScene("home");
                }
